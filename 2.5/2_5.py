...
def em_algorithm(seed_val, samples, num_clusters, max_num_iter=100):
    ...
    # 1. compute responsibilities r_nk
    N = samples.shape[0]
    K = num_clusters
    r = np.zeros((N, K))
    ...
    for n in range(N):
        for k in range(K):
            visit_list = [tm.clusters[k].root]
            while len(visit_list) != 0:
                cur_node = visit_list[0]
                visit_list = visit_list[1:] + cur_node.descendants
                parent_node = cur_node.ancestor

                if cur_node == tm.clusters[k].root:
                    posterior[n, k] *= cur_node.cat[samples[n][int(cur_node.name)]]
                else:
                    posterior[n, k] *= cur_node.cat[samples[n][int(parent_node.name)]][
                        samples[n][int(cur_node.name)]]
        prior[n] *= np.sum(posterior[n] * tm.pi)
    # r = (np.array(posterior) + sys.float_info.epsilon) / (np.array([prior]).T + K * sys.float_info.epsilon)
    # r = (tm.pi * np.array(posterior) + np.finfo(float).eps) / (np.array([prior]).T + K * np.finfo(float).eps)
    r = (tm.pi * np.array(posterior) + sys.float_info.epsilon) / (np.array([prior]).T + K * sys.float_info.epsilon)
